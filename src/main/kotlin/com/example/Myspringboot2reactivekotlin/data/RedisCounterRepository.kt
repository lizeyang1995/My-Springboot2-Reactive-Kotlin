...
class RedisCounterRepository(private val redisTemplate: ReactiveRedisTemplate<String, String>,
    ...
    override fun up(): Mono<Long> = redisTemplate.createMono { it.numberCommands().incr(key) }

    override fun down(): Mono<Long> = redisTemplate.createMono { it.numberCommands().decr(key) }

    override fun get(): Mono<Long> = redisTemplate.createMono { it.numberCommands().incrBy(key, 0L) }
}